...
efficiency->Draw("P");
 ...
 TLegend *leg = new TLegend(0.608739,0.3024784,0.8085924,0.4023343,NULL,"brNDC");
  ...
  ltx->DrawLatex(WP-8*add, 0.42+up, Form("Eff(WP) = %.2f", sigmoid->Eval(WP)));
  ltx->DrawLatex(WP-8*add, 0.35+up, Form("WP = %.f V", WP));
  ltx->DrawLatex(WP-8*add, 0.27+up, Form("knee = %.f V", knee));
  ltx->DrawLatex(WP-8*add, 0.20+up, Form("HV(50%) = %.f V", p3));
  ...
  string outfile_root = outfile + "" + fileIn + ".root";
