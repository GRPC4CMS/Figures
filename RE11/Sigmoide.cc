...
void Sigmoide(string sName){
  ...
  fNameStat = sName + "_stat.root";
  TFile *_file1 = TFile::Open(fNameStat.c_str());
  //TCanvas* canv = (TCanvas*) _file0->Get("Comparaison/Method1/1_3.000000_0_Gaussian + constante_al;1");
  TGraphAsymmErrors* EfficiencyStat = (TGraphAsymmErrors*) _file1->Get("Gaussian + constante al Efficiency, threshold = 220.00mV, +-3.00;1");
  ...
  for (int i = 0; i < Efficiency->GetN(); i++){
    ...
    cout << "HV = " << x << " eff = "  << y-errorY/2 << " errorY = " << errorY/2 << " errorYhigh_stat = " << errorYhigh_stat << " errorYlow_stat = " << errorYlow_stat << endl;
